                ...
                <div class="max-w-lg w-full lg:max-w-xs">
                    <form id="search-form" method="GET" action="{{ route('recipes.index') }}" class="relative">
                        ...
                        <div class="relative">
                            ...
                            <input id="search" name="search" class="block w-full pl-10 pr-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md leading-5 bg-white dark:bg-gray-700 placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Zoek recepten..." type="search" value="{{ request('search') }}">
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                @auth
                    ...
                    <div class="ml-3 relative">
                        <button id="account-menu-button" type="button" class="flex items-center text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                            <img class="h-8 w-8 rounded-full" src="{{ Auth::user()->profile_photo_url ?? 'https://ui-avatars.com/api/?name=' . urlencode(Auth::user()->name) . '&color=7F9CF5&background=EBF4FF' }}" alt="User Avatar">
                        ...
                        <div id="account-menu" class="hidden origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white dark:bg-gray-700 ring-1 ring-black ring-opacity-5">
                            <div class="py-1">
                                ...
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <button type="submit" class="block w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600">Log Out</button>
                                </form>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    document.getElementById('search').addEventListener('input', function() {
        ...
        `;
                ...
                const paginationContainer = document.querySelector('.pagination-container'); // Add a container for pagination in index.blade.php
